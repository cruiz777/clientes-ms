...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class VendedorController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllVendedorQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetVendedorByIdQuery(id));
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] VendedorRequest request)
        {
            var result = await _mediator.Send(new CreateVendedorCommand(request));
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] VendedorRequest request)
        {
            var result = await _mediator.Send(new UpdateVendedorCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteVendedorCommand(id));
            return Ok(result);
        }
