...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class EstadoEmpresaController : ControllerBase
    {
        private readonly IMediator _mediator;

        // Constructor con inyección de dependencia del Mediator
        public EstadoEmpresaController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var result = await _mediator.Send(new GetAllEstadoEmpresaQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetEstadoEmpresaByIdQuery(id));
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] EstadoEmpresaRequest request)
        {
            var result = await _mediator.Send(new CreateEstadoEmpresaCommand(request));
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] EstadoEmpresaRequest request)
        {
            var result = await _mediator.Send(new UpdateEstadoEmpresaCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteEstadoEmpresaCommand(id));
            return Ok(result);
        }
