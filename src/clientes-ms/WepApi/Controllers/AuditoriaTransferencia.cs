...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class AuditoriaTransferenciaController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllAuditoriaTransferenciaQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetAuditoriaTransferenciaByResumen()); // Envía la query a su handler correspondiente
            return Ok(result); // Devuelve la respuesta con estado 200
        }
