...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class CantonesController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllCantonesQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetCantonesByIdQuery(id));
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CantonesRequest request)
        {
            var result = await _mediator.Send(new CreateCantonesCommand(request));
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] CantonesRequest request)
        {
            var result = await _mediator.Send(new UpdateCantonesCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteCantonesCommand(id));
            return Ok(result);
        }
