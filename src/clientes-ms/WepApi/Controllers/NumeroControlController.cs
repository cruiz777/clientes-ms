...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class NumeroControlController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllNumeroControlQuery()); // Envía la query a su handler correspondiente
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] NumeroControlRequest request)
        {
            var result = await _mediator.Send(new UpdateNumeroControlCommand(id, request));
            return Ok(result);
        }
