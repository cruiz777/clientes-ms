...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class CiudadesController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllCiudadesQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetCiudadesByIdQuery(id));
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] CiudadesRequest request)
        {
            var result = await _mediator.Send(new CreateCiudadesCommand(request));
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] CiudadesRequest request)
        {
            var result = await _mediator.Send(new UpdateCiudadesCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteCiudadesCommand(id));
            return Ok(result);
        }
