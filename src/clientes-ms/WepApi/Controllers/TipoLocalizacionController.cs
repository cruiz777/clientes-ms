...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class TipoLocalizacionController : ControllerBase
    {
        private readonly IMediator _mediator;

        // Constructor con inyección de dependencia del Mediator
        public TipoLocalizacionController(IMediator mediator)
        {
            _mediator = mediator;
        }
        ...
        {
            var result = await _mediator.Send(new GetAllTipoLocalizacionQuery()); // Envía la query a su handler correspondiente
        ...
        {
            var result = await _mediator.Send(new GetTipoLocalizacionByIdQuery(id));
            return Ok(result);
        }
        ...
        [HttpPost]
        public async Task<IActionResult> Create([FromBody] TipoLocalizacionRequest request)
        {
            var result = await _mediator.Send(new CreateTipoLocalizacionCommand(request));
            return Ok(result);
        }
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] TipoLocalizacionRequest request)
        {
            var result = await _mediator.Send(new UpdateTipoLocalizacionCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteTipoLocalizacionCommand(id));
            return Ok(result);
        }
