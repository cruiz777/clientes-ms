...
namespace clientes_ms.WebApi.Controllers
{
    ...
    public class ExampleController : ControllerBase
    {
        ...
        {
            var result = await _mediator.Send(new GetAllSectorQuery()); // Envía la query a su handler correspondiente
        ...
        [HttpPut("{id}")]
        public async Task<IActionResult> Update(long id, [FromBody] ExampleRequest request)
        {
            var result = await _mediator.Send(new UpdateExampleCommand(id, request));
            return Ok(result);
        }
        ...
        {
            var result = await _mediator.Send(new DeleteSectorCommand(id));
            return Ok(result);
        }
